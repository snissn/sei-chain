...
func TestDataHashOverwriteIgnoresStaleTxHashMetadata(t *testing.T) {
	data := Data{Txs: Txs{Tx("alpha"), Tx("beta")}}
	require.NoError(t, data.SetTxHashes(data.Txs.Hashes()))

	originalHash := data.Hash(false)
	data.Txs[0] = Tx("omega")
	expectedHash := data.HashFromTxs()
	require.NotEqual(t, originalHash, expectedHash)

	require.Equal(t, expectedHash, data.Hash(true))
	require.Equal(t, expectedHash, data.Hash(false))
}
